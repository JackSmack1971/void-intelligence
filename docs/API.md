...
Void Intelligence is primarily a server-side engine interacted with via Next.js Server Actions. All endpoints enforce robust sanitization, payload validations, and cached asynchronous data reads.
...
- **Behavior**:
    - Validates query structure and input length constraints.
    - Initializes the `runGoA` engine.
    - Triggers background triplet extraction.
    - Stores the conversation in SQLite.

### `syncKg()`
Synchronizes the local Knowledge Graph with server-side triplets.
- **Returns**: `Promise<{ success: boolean; data?: Triplet[]; error?: string }>`

### `getTripletsForExport()`
Fetches all active SQLite relations for PBKDF2/AES-GCM client-side export packages.
- **Returns**: `Promise<{ success: boolean; data?: Triplet[]; error?: string }>`

### `importSelectedTriplets(triplets: Triplet[])`
Imports validated, decrypted, and de-duplicated user relationships back into SQLite via ACID batch transactions.
- **Parameters**: `triplets` (array of `Triplet` items)
- **Returns**: `Promise<{ success: boolean; error?: string }>`

## GoA Engine (`lib/goa/index.ts`)
Unified barrel re-exports expose the GoA orchestrator and debate engines from `@/lib/goa`:
